...
class Board(Point):
    ...
    def move(self, point):
        ...
        if (isSuicide):
            # print 'Sorry. Suicidal moves are not allowed.' 
            return None 
...
def userMove(board):
    while True:
        print '\n'
        text = raw_input('Your move (XY): ').strip()
        if text == 'p':
            return 'PASS'
        if text == 'q':
            exit()
        try:
            x, y = [int(i) for i in text]
        except ValueError:
            continue
...
def printIntro():
    ...
    print ' To move, select XY where X is down and Y is right.  '
    print ' Type "p" to pass a move and "q" to quit.            '
    print ' The game ends after 10 turns.                       '
    print '                                                     '
